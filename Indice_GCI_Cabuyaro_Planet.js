...
/*El siguiente script crea visualizaciones en video con los cambios en el terreno del indice GCI que se
presentan en una coleccion de imagenes multiespectrales PLANET de 4,7 m de resolucion en un poligono 
...
var coleccion = nicfi.filter(ee.Filter.date('2021-09-01','2023-10-30'))
...
/*Se define la variable de parametros para la generacion del indice para cada imagen de la 
coleccion y se crea la funcion de cada imagen recortada al area del poligono de la geometria dada*/

var parametrosgci = {min:-0.5, max:10, palette: ['blue', 'black', 'white', 'yellow', 'green', 'red']}
var gci = coleccion.map(function (e){return e.expression('((nir)/(green))-(L)',
  {
    nir : e.select('N').divide(10000),
    green : e.select('G').divide(10000),
    L : 1,
    }).clip(geometry)})
var visgci = gci.map(function (e){return e.visualize(parametrosgci)})


/*Se establece la variable de visualizacion por imagenes por segundo y se crea la visualizacion del
video en la consola con la funcion ui.Thumbnail para el indice y se imprime para
ver el video en consola*/

var viscontrol = {framesPerSecond: 3}
var timeserie = ui.Thumbnail(visgci, viscontrol)

print('gci', timeserie)


/*Se adiciona a la coleccion de imagenes la banda del indice GCI recortada a la geometria, se definen 
los parametros de visualizacion y se visualiza en el mapa que se genera de la coleccion de imagenes*/

var app = function (image){
  var gci = image.expression('((nir)/(green))-(L)',
  {
    nir : image.select('N').divide(10000),
    green : image.select('G').divide(10000),
    L : 1,
    })
...
var visParams = {bands:["GCI"], min: -0.5, max:10, palette: ["blue", "black", "white", "yellow", "green", "red"]}
Map.addLayer(coleccion1, visParams, "GCI_coleccion")
...
print('video coleccion gci cabuyaro', gif1)


/*Se crea una etiqueta a partir de un punto que se defina mediante un click en el mapa
la cual va a presentar la grafica el indice de vegetacion GCI para el sitio que se define en el click*/
...
Map.onClick(function(click){
  ...
  Map.layers().set(1, ui.Map.Layer(punto, {color: 'red', pointSize: 20}))
  Map.centerObject(punto, 14)
    var grafico = ui.Chart.image
    .series({
      imageCollection: coleccion1.select("GCI"),
      region: punto, 
    ...
    .setOptions({
      ...
      vAxis: {title: 'Valores índice de vegetación GCI'},
      hAxis: {title: 'Mes/Año'},
      lineWidth: 2,
      colors: ['green']
    })
