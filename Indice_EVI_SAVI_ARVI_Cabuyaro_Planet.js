...
/*El siguiente script crea visualizaciones en video con los cambios en el terreno y en los indices EVI,
SAVI, ARVI que se presentan en una coleccion de imagenes multiespectrales PLANET de 4,7 m
de resolucion en un poligono creado por geometria que es un area de cultivo en Cabuyaro Meta*/

/*Traer la coleccion de imagenes PLANET con un filtro de fechas, un filtro con los parametros de 
visualizacion en RGB y recortadas al area del poligono para cada una de las imagenes de la coleccion
al final se imprime la coleccion para saber cuantas imagnes tiene y la visualiza en el mapa*/

var nicfi = ee.ImageCollection('projects/planet-nicfi/assets/basemaps/americas');
var coleccion = nicfi.filter(ee.Filter.date('2021-12-01','2023-12-30'))
.map(function(i){return i.select(['B','G','R','N']).clip(geometry)})
print(coleccion)

var visparam = {bands:['R','G','B'], min: 394, max:1876, gamma: 1.7}
...
/*Se define la variable de parametros para la generacion de los indices GCI, SAVI, ARVI
para cada imagen de la coleccion y se crea la funcion para cada imagen
recortada al area del poligono de la geometria dada*/

var parametrosevi = {min: -0.5, max: 0.9, palette: ['blue', 'black', 'white', 'yellow', 'green', 'red']}
var evi = coleccion.map(function (g){return g.expression('(2.5)*((nir-red)/(nir+(red*C)-(B*blue)+L))',
  {
    nir : g.select('N').divide(10000),
    red : g.select('R').divide(10000),
    blue : g.select('B').divide(10000),
    C : 6,
    B : 7.5,
    L : 1
  }).clip(geometry)})
var visevi = evi.map(function (g){return g.visualize(parametrosevi)})

var parametrosavi = {min:-0.5, max:0.9, palette: ['blue', 'black', 'white', 'yellow', 'green', 'red']}
var savi = coleccion.map(function (a){return a.expression('(1+L)*((nir-red)/(nir+red+L))',
  {
    nir : a.select('N').divide(10000),
    red : a.select('R').divide(10000),
    L : 0.5
  }).clip(geometry)})
...
var arvi = coleccion.map(function (b){return b.expression('(nir-(2*red)+blue)/(nir+(2*red)+blue)',
  {
    nir : b.select('N').divide(10000),
    red : b.select('R').divide(10000),
    blue : b.select('B').divide(10000),
  }).clip(geometry)})
...
var viscontrol = {framesPerSecond: 3}
var timeserie1 = ui.Thumbnail(visevi, viscontrol)
var timeserie2 = ui.Thumbnail(visavi, viscontrol)
var timeserie3 = ui.Thumbnail(visarvi, viscontrol)
print('evi', timeserie1)
print('savi', timeserie2)
print('arvi', timeserie3)
 ...
 var evi = image.expression('(2.5)*((nir-red)/(nir+(red*C)-(B*blue)+L))',
  {
    ...
    blue : image.select('B').divide(10000),
    C : 6,
    B : 7.5,
    L : 1
  })  
...
var visParams1 = {bands:["EVI"], min: -0.5, max:0.9, palette: ["blue", "black", "white", "yellow", "green", "red"]}
Map.addLayer(coleccion1, visParams1, "EVI_coleccion")

var visParams2 = {bands:["SAVI"], min: -0.5, max:0.9, palette: ["blue", "black", "white", "yellow", "green", "red"]}
Map.addLayer(coleccion1, visParams2, "SAVI_coleccion")

var visParams3 = {bands:["ARVI"], min: -0.5, max:0.9, palette: ["blue", "black", "white", "yellow", "green", "red"]}
Map.addLayer(coleccion1, visParams3, "ARVI_coleccion")
...
var gif1 = coleccion1.getVideoThumbURL(visParams1)
print('video coleccion evi cabuyaro', gif1)

var gif2 = coleccion1.getVideoThumbURL(visParams2)
print('video coleccion savi cabuyaro', gif2)

var gif3 = coleccion1.getVideoThumbURL(visParams3)
...
Map.onClick(function(click){
  ...
  Map.layers().set(1, ui.Map.Layer(punto, {color: 'black', pointSize: 10}))
  Map.centerObject(punto, 14)
    var grafico = ui.Chart.image
    .series({
      imageCollection: coleccion1.select("EVI","SAVI","ARVI"),
      // imageCollection: coleccion1.select("NDVI","SAVI","ARVI","GCI","EVI"),
      region: punto, 
      reducer: ee.Reducer.mean(), 
      scale: 4.7,
      xProperty: 'system:time_start'
    })
    .setSeriesNames(["EVI","SAVI","ARVI"])
    // .setSeriesNames(["NDVI","SAVI","ARVI","GCI","EVI"])
    .setOptions({
      title: 'Distribución de valores promedio índices de vegetación',
